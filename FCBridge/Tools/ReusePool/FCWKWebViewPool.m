...
@interface FCWKWebViewPool()
@property(nonatomic, strong, readwrite) dispatch_semaphore_t lock;
@property(nonatomic, strong, readwrite) NSMutableArray<__kindof FCWKWebView *> *visiableWebViewSet;
@property(nonatomic, strong, readwrite) NSMutableArray<__kindof FCWKWebView *> *reusableWebViewSet;
@end

@implementation FCWKWebViewPool
...
+ (void)prepareWebView {
    [[FCWKWebViewPool sharedInstance] _prepareReuseWebView];
}

+ (instancetype)sharedInstance {
    static dispatch_once_t once;
    static FCWKWebViewPool *webViewPool = nil;
    dispatch_once(&once,^{
        webViewPool = [[FCWKWebViewPool alloc] init];
    });
    return webViewPool;
}
...
- (void)_tryCompactWeakHolders {
    ...
    NSMutableSet<FCWKWebView *> *shouldreusedWebViewSet = [NSMutableSet set];

    for (FCWKWebView *webView in _visiableWebViewSet) {
        if (!webView.holderObject) {
            [shouldreusedWebViewSet addObject:webView];
        }
    }

    for (FCWKWebView *webView in shouldreusedWebViewSet) {
        [webView webViewEndReuse];
...
- (void)_prepareReuseWebView {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        FCWKWebView *webView = [FCWKWebView webView];
